...
class Pipeline:
    ...
    def run_detection(self, reinfer_mode: str = 'auto', use_crop: bool = False):
        """
        Stage 1: Run YOLO segmentation to produce mask JSON files.

        Args:
            reinfer_mode: 'on', 'off', or 'auto'
            use_crop: Enable crop-based multi-scale inference
        """
        logger.info("=" * 80)
        logger.info("Stage 1: YOLO Segmentation Inference")
        if use_crop:
            logger.info("Mode: Crop-based multi-scale inference")
        else:
            logger.info("Mode: Standard inference")
        logger.info("=" * 80)
        ...
        self.run_yolo_inference(image_ids, effective_mode, use_crop=use_crop)

    def run_yolo_inference(self, image_ids: List[str], reinfer_mode: str, use_crop: bool = False):
        """
        ...
        try:
            if use_crop:
                from .yolo_inference import YOLOSegmenterWithCrop as SegmenterClass
            else:
                from .yolo_inference import YOLOSegmenter as SegmenterClass
        ...
        with Timer("YOLO Inference"):
            ...
            if use_crop:
                crop_config = yolo_config.get('crop', {})
            ...
            inferencer = SegmenterClass(**common_params)
...
def main():
    ...
    parser.add_argument('--use-crop', action='store_true',
                       help='Enable crop-based multi-scale inference for improved fine-scale detection')
    ...
    try:
        if args.command == 'sfm':
            ...
            pipeline.run_detection(reinfer_mode=reinfer_mode, use_crop=args.use_crop)
